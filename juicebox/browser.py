...
class Browser(object):
    """Object representing the a juicebox browser"""

    def __init__(self, config):
        """Initialize the juicebox browser object.

        :param: config - A dictionary specifying the browser configuration.  This will be converted to JSON and passed
                to juicebox.js  "juicebox.createBrowser(div, config)" as described in the juicebox.js documentation.
        :type dict
        """

        # Check for minimal juicebox-jupyter.js requirements (the only required field for all tracks is url, which must be a string)
    ...
    def show(self, navbar=False):
        """Create an juicebox-jupyter.js "Browser" instance on the front end."""
        if navbar: navbar_html = '<div class="juicebox-jupyter-navbar"></div>'
        else: navbar_html = ''

        display(HTML(f"""{navbar_html}<div id="{self.juicebox_id}"></div>"""))
    ...
    def load_track(self, track):
        """
        Load a track.  Corresponds to the juicebox-jupyter.js Browser function loadTrack (see https://github.com/igvteam/juicebox-jupyter.js/wiki/Browser-Control-2.0#loadtrack).

        :param  track: A dictionary specifying track options.  See https://github.com/igvteam/juicebox-jupyter.js/wiki/Tracks-2.0.
        :type dict
        """
    ...
    def on(self, event_name, cb):
        """
        Subscribe to an juicebox-jupyter.js event.
    ...
    def remove(self):
        """
        Remove the juicebox-jupyter.js Browser instance from the front end.  The server Browser object should be disposed of after calling
        this method.
        """
        self._send({
            "id": self.juicebox_id,
