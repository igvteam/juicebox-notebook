...
function handle_message(msg, comm) {
    ...
    if (method === "createBrowser") createBrowser(div, data.options)
    ...
    function getBrowser(id) {
        return juicebox - jupyter.browserCache[id]
    }
    ...
    async function remove(div) {
        try {
            delete juicebox - jupyter.browserCache[id]
...
async function prepare_urls_nb(config) {
    ...
    for (let urlKey of to_check) {
        if (config[urlKey] && is_local(config[urlKey])) {
            config[urlKey] = `${location.origin}${base_path}files/${nb_dir}${url}`
        }
    }
}
...
async function prepare_urls_lab(config) {
    ...
    for (let urlKey of to_check) {
        ...
        if (url && resolver && resolver.isLocal(url)) {
...
export function init_comm_nb() {
    ...
    Jupyter.notebook.kernel.comm_manager.register_target('jbcomm', (comm) => {
...
function init_comm_lab(notebook_tracker) {
    ...
    notebook_tracker.activeCellChanged.connect(() => {
        ...
        current_panel.context.sessionContext.ready.then(() => {
            ...
            kernel.registerCommTarget('jbcomm', (comm) => {
...
const juicebox_plugin = [
    {
        id: '@igvteam/juicebox-jupyter',
        autoStart: true,
        optional: [INotebookTracker],
        activate: function (app, tracker) {
            console.log('JupyterLab extension @igvteam/juicebox-jupyter is activated!')
...
export default juicebox_plugin
